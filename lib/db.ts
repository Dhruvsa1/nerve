...
// Serverless-friendly singleton pool. Connects as the schema-scoped `nerve_app`
// role through the Supabase transaction pooler (see .env.local). The role can ONLY
// touch the `nerve` schema — every other tenant's data is unreachable.
const g = globalThis as unknown as { _nervePool?: Pool }

export function pool(): Pool {
  if (!g._nervePool) {
    g._nervePool = new Pool({
